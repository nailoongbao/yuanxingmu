...
新增独立的 OpenClaw 实例：在原生聊天界面里使用自己的模型、读取指定文本资料。聊天粘贴和工作产物从一开始就按私密资料处理；换会话或正常重启不清除权限。默认没有发送位置。

[按步骤开始使用](docs/openclaw-quickstart.zh-CN.md)。这仍是 Linux/WSL 预览版，需要终端安装。所选模型服务会看到聊天和使用的资料。
