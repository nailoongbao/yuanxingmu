...
**main 新增：** [smolagents](docs/sdk-runtime.zh-CN.md)、[LangGraph](docs/langgraph-runtime.zh-CN.md) 和 [OpenAI Agents](docs/openai-agents-runtime.zh-CN.md) 的完整会话入口，复用已有工作的隔离、防护和恢复记录；有既定自动范围时，可自动发送消息、上传文本和提交表单。OpenAI Agents 支持固定助手交接，报告同时保留两轮 GLM 失败与声明兼容调整后的运行复测。这三条开发入口尚未进入上面的 a2 下载包，仅覆盖各自固定流程。
