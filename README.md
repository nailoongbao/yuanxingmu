...
**New on main:** run complete [smolagents](docs/sdk-runtime.zh-CN.md), [LangGraph](docs/langgraph-runtime.zh-CN.md), [OpenAI Agents](docs/openai-agents-runtime.zh-CN.md), or [PydanticAI](docs/pydantic-ai-runtime.zh-CN.md) sessions inside the same host defenses, with persistent recovery and optional automatic messages, uploads and forms. OpenAI Agents includes a fixed native handoff; its report preserves failed attempts and the schema compatibility retest. The [two PydanticAI trials](docs/evidence/pydantic-ai-runtime-2026-09-13/REPORT.zh-CN.md) preserve an unknown-tool failure and the subsequent completed run. These developer entries are not included in the frozen a2 downloads and cover fixed workflows only.
