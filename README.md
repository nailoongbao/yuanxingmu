...
**New on main:** run complete [smolagents](docs/sdk-runtime.zh-CN.md), [LangGraph](docs/langgraph-runtime.zh-CN.md), [OpenAI Agents](docs/openai-agents-runtime.zh-CN.md), [PydanticAI](docs/pydantic-ai-runtime.zh-CN.md), or [Google ADK](docs/google-adk-runtime.zh-CN.md) sessions inside the same host defenses, with persistent recovery and optional automatic messages, uploads and forms. OpenAI Agents and Google ADK include fixed native handoffs that retain the original authority and budget. The [Google ADK trial](docs/evidence/google-adk-runtime-2026-09-13/REPORT.zh-CN.md) records three completed actions, verification against host-owned originals, and no duplicate effects on reopen. These five developer entries are not included in the frozen a2 downloads and cover fixed workflows only.
