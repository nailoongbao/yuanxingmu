...
**New on main:** run complete [smolagents](docs/sdk-runtime.zh-CN.md), [LangGraph](docs/langgraph-runtime.zh-CN.md), or [OpenAI Agents](docs/openai-agents-runtime.zh-CN.md) sessions inside the same host defenses, with persistent recovery and optional automatic messages, uploads and forms. OpenAI Agents includes a fixed native handoff; its report preserves two failed GLM attempts and the schema compatibility runtime retest. These developer entries are not included in the frozen a2 downloads and cover fixed workflows only.
