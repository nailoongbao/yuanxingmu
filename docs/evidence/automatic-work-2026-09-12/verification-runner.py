...
def main():
    ...
    modules = ['test_yuanxingmu_'+name for name in (
        ...
        ('action_automation','actions','authority','automatic_broker','input_containment',
    ...
    report = {'recorded_at':datetime.now(timezone.utc).isoformat(), 'commit':args.commit,
              'scope':'Component regressions. Synthetic judge replies and local HTTP receipts; no live model or native WebUI session.',
