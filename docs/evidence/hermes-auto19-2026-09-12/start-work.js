async page => {
  ...
  const responsePromise=page.waitForResponse(r=>r.request().method()==='POST'&&r.url().endsWith('/start'),{timeout:20000});
  await card.getByRole('button',{name:"启动工作",exact:true}).click();
