"""Real LangGraph loops with a local host journal and real Broker Unix RPC.

Model replies are deterministic fixtures. These tests exercise StateGraph,
native ToolNode dispatch and framework checkpoint recovery, not inference or
...
HAS_SDK = all(importlib.util.find_spec(name) is not None for name in ("langgraph", "langchain_core"))
REQUIRES_SDK = unittest.skipUnless(sys.platform.startswith("linux") and HAS_SDK,
                                  "Requires Linux and the pinned LangGraph SDK environment")
COMPLETE = "LOCAL-LANGGRAPH-LOOP-COMPLETE"
...
class LanggraphFixture:
    ...
    def setUp(self):
        from yuanxingmu.adapters import langgraph_runtime
        self.runtime = langgraph_runtime
        self.addCleanup(patch.stopall)
        patch.dict(os.environ, {"LANGCHAIN_TRACING_V2": "false", "LANGSMITH_TRACING": "false",
                                "OTEL_SDK_DISABLED": "true"}).start()
        temporary = tempfile.TemporaryDirectory(prefix="yxm-langgraph-loop-")
        ...
        self.client = NativeTools(str(endpoint), "host-langgraph-session")
        ...
        self.config = {"version": 1, "framework": "langgraph", "session_id": self.client.session_id,
    ...
    @staticmethod
    def proposal(body="LOCAL-LANGGRAPH-PROGRESS", *, target="chat"):
...
@REQUIRES_SDK
class LanggraphRuntimeTests(LanggraphFixture, unittest.TestCase):
    def test_native_graph_and_singleton_tool_nodes_reach_model_and_broker(self):
        from langgraph.checkpoint.base import BaseCheckpointSaver
        from langgraph.graph import StateGraph
        from langgraph.prebuilt import ToolNode
        from langgraph.pregel import Pregel
        native_compiles, native_calls, durabilities = [], [], []
        compile_graph, parse_input, invoke_graph = StateGraph.compile, ToolNode._parse_input, Pregel.invoke

        def observed_compile(builder, *args, **kwargs):
            native_compiles.append(kwargs.get("checkpointer", args[0] if args else None))
            return compile_graph(builder, *args, **kwargs)

        def observed_parse(node, value):
            calls, input_type = parse_input(node, value)
            native_calls.append(deepcopy(calls))
            return calls, input_type

        def observed_invoke(graph, *args, **kwargs):
            durabilities.append(kwargs.get("durability"))
            return invoke_graph(graph, *args, **kwargs)

        self.responses.extend([
            completion(tool_call("host-read", "yuanxingmu_read", {"resource": "note"})),
        ...
        with patch.object(StateGraph, "compile", observed_compile), patch.object(ToolNode, "_parse_input", observed_parse), \
                patch.object(Pregel, "invoke", observed_invoke):
            result = self.runtime.run_session(deepcopy(self.config))
        self.assertEqual(result["answer"], COMPLETE)
        self.assertEqual(result["framework"], "langgraph")
        self.assertTrue(native_compiles)
        self.assertTrue(all(isinstance(saver, BaseCheckpointSaver) for saver in native_compiles))
        self.assertEqual(len(native_calls), 3)
        self.assertTrue(all(len(calls) == 1 for calls in native_calls))
        self.assertEqual([calls[0]["id"] for calls in native_calls],
                         ["host-read", "host-proposal-one", "host-proposal-two"])
        self.assertTrue(durabilities)
        self.assertTrue(all(value == "sync" for value in durabilities))
        ...
        self.assertIn("host-proposal-two", json.dumps(self.upstream_requests[-1]["body"]["messages"]))
        self.assertEqual(len(self.actions()), 2)
        self.assertTrue(all(action["status"] == "pending" for action in self.actions()))
    ...
    def test_empty_native_record_set_cannot_restart_model_during_resume(self):
        self.responses.append(final_response())
        self.runtime.run_session(deepcopy(self.config))
        checkpoint = Path(self.config["checkpoint"])
        saved = json.loads(checkpoint.read_bytes())
        saved.update(latest=None, records=[])
        raw = json.dumps(saved).encode("utf-8")
        checkpoint.write_bytes(raw)
        count = len(self.requests)
        with self.assertRaisesRegex(ValueError, "checkpoint_incomplete"):
            self.runtime.run_session({**self.config, "resume": True})
        self.assertEqual(checkpoint.read_bytes(), raw)
        self.assertEqual(len(self.requests), count)
        self.assertEqual(self.actions(), [])

    def test_invalid_member_rejects_the_entire_tool_batch_before_broker(self):
    ...
    def test_multiple_model_choices_fail_before_tool_dispatch(self):
        response = completion(tool_call("host-choice", "yuanxingmu_propose_action", self.proposal()))
    ...
    def test_worker_error_output_hides_prompt_and_upstream_body(self):
        prompt_secret = "SYNTHETIC-PRIVATE-LANGGRAPH-PROMPT-DO-NOT-PRINT"
        ...
        result = subprocess.run([sys.executable, "-B", "-m", "yuanxingmu.adapters.langgraph_runtime",
